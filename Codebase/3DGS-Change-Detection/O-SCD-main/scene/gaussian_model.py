...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int, active_sh_degree : int = 0):
    ...
    def training_setup(self, training_args):
        ...
        l = [
            {'params': [self._xyz], 'lr': training_args.position_lr_init * 2 * self.spatial_lr_scale, "name": "xyz"},
            {'params': [self._features_dc], 'lr': training_args.feature_lr * 2, "name": "f_dc"},
            {'params': [self._features_rest], 'lr': training_args.feature_lr * 2 / 20.0, "name": "f_rest"},
    ...
    def training_setup_update(self, training_args): #from FastGS 
        self.percent_dense = training_args.percent_dense
        self.xyz_gradient_accum = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")
    ...
    def training_setup_change(self, training_args):
        self.percent_dense = training_args.percent_dense
        self.xyz_gradient_accum = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")
        self.denom = torch.zeros((self.get_xyz.shape[0], 1), device="cuda")
        ...
        self.xyz_scheduler_args = get_expon_lr_func(lr_init=training_args.position_lr_init*self.spatial_lr_scale,
                                                    lr_final=training_args.position_lr_final*self.spatial_lr_scale,
                                                    lr_delay_mult=training_args.position_lr_delay_mult * 5,
                                                    max_steps=training_args.position_lr_max_steps)
    ...
    def optimizer_step(self, iteration): #from FastGS 
    ...
    def densification_postfix_fastgs(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_tmp_radii):
        d = {"xyz": new_xyz,
        "f_dc": new_features_dc,
        "f_rest": new_features_rest,
        "opacity": new_opacities,
    ...
    def densify_and_split_fastgs(self, metric_mask, filter, N=2):
        ...
        self.densification_postfix_fastgs(new_xyz, new_features_dc, new_features_rest, new_opacity, new_scaling, new_rotation, new_tmp_radii)

        prune_filter = torch.cat((selected_pts_mask, torch.zeros(N * selected_pts_mask.sum(), device="cuda", dtype=bool)))
        self.prune_points_fastgs(prune_filter)
    ...
    def densify_and_clone_fastgs(self, metric_mask, filter):
        ...
        self.densification_postfix_fastgs(new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_tmp_radii)

    def densify_and_prune_fastgs(self, max_screen_size, min_opacity, extent, radii, args, importance_score = None, pruning_score = None, constrain_mask = None):
        ...
        if constrain_mask is not None:
            grad_vars[~constrain_mask] = 0.0
            grads_abs[~constrain_mask] = 0.0
        ...
        prune_mask = (self.get_opacity < min_opacity).squeeze()
        if constrain_mask is not None:
            if constrain_mask.shape[0] < prune_mask.shape[0]:
                padding = torch.zeros(prune_mask.shape[0] - constrain_mask.shape[0], 
                                     dtype=torch.bool, device=constrain_mask.device)
                constrain_mask = torch.cat([constrain_mask, padding])
            prune_mask = torch.logical_and(prune_mask, constrain_mask)
        if max_screen_size:
            big_points_vs = self.max_radii2D > max_screen_size
            big_points_ws = self.get_scaling.max(dim=1).values > 0.1 * extent
            prune_mask = torch.logical_or(torch.logical_or(prune_mask, big_points_vs), big_points_ws)
    ...
    def add_densification_stats_fastgs(self, viewspace_point_tensor, update_filter):
        self.xyz_gradient_accum[update_filter] += torch.norm(viewspace_point_tensor.grad[update_filter,:2], dim=-1, keepdim=True)
