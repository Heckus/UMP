...
def render_change(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, mult=0.5, scaling_modifier = 1.0, override_color = None, get_flag=None, metric_map = None):
    """
    Render the scene. 
...
# This would give slightly better SCD performance, but would be result in a slightly slower FPS

# def render_change(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scores = None, scaling_modifier = 1.0, override_color = None):
#     """
#     Render the scene. 
...
#     shs = None
#     colors_precomp = None
#     shs = pc._features_dc.repeat(1,1,3)
#     # if override_color is None:
#     #     if pipe.convert_SHs_python:
#     #         shs_view = pc.get_features.transpose(1, 2).view(-1, 3, (pc.max_sh_degree+1)**2)
#     #         dir_pp = (pc.get_xyz - viewpoint_camera.camera_center.repeat(pc.get_features.shape[0], 1))
...
#             "kernel_times": kernel_times}
    ...
    raster_settings = GaussianRasterizationSettingsPixel(
        image_height=int(viewpoint_camera.image_height),
        image_width=int(viewpoint_camera.image_width),
        tanfovx=tanfovx,
        tanfovy=tanfovy,
    ...
    rasterizer = GaussianRasterizerPixel(raster_settings=raster_settings)
    ...
    raster_settings = GaussianRasterizationSettings(
        ...
        viewmatrix=viewpoint_camera.world_view_transform,
        projmatrix=viewpoint_camera.full_proj_transform,
        sh_degree=0, # No SH-based color in change rendering
        campos=viewpoint_camera.camera_center,
        mult = mult,
        prefiltered=False,
        debug=pipe.debug,
