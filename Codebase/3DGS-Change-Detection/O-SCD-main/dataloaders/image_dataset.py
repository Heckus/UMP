...
from utils.utils_fly import get_image_names
...
class ImageDataset:
    ...
    """
    def __init__(self, args: Namespace, instance: str = 'inf'):
        self.images_dir = os.path.join(args.source_path, "inference_scene", "images") if instance == 'inf' else os.path.join(args.source_path, "reference_scene", "images")
        ...
        self.downsampling = args.resolution 
        ...
        # Load COLMAP data
        colmap_path = None if instance == 'inf' else os.path.join(args.source_path, "reference_scene", "sparse/0")
        if colmap_path is not None:
            self.load_colmap_data(colmap_path)
    ...
    def _load_image(self, image_path, mode=cv2.IMREAD_COLOR):
        ...
        # print(f"Loading image: {image_path}, original shape: {image.shape if image is not None else 'None'}")
        if image is None:
    ...
    def load_colmap_data(self, colmap_folder_path):
        ...
        for image_id, image in images.items():
            ...
            FovY = focal2fov(focal_y, camera.height)
            FovX = focal2fov(focal_x, camera.width)
            ...
            if image.name in self.infos:
                ...
                self.infos[name]["FovX"] = FovX
                self.infos[name]["FovY"] = FovY
